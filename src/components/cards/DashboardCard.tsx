...
export default function DashboardCard() {
  ...
  const handleScanResult = (value: string) => {
    setScanResult(value);
    setOpenScan(false);
    // Lakukan proses sesuai kebutuhan, misal request ke backend atau tampilkan alert
    alert("Hasil scan tiket: " + value);
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-5 md:gap-6">
      ...
      {/* Button Scan Tiket */}
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
        <div className="flex items-center justify-center w-full h-30 bg-gray-100 rounded-xl dark:bg-gray-800">
          <LuScanLine className="text-gray-800 size-6 dark:text-white/90" />
        </div>

        <div className="flex items-end justify-between mt-5">
          <div className="w-full">
            <Button className="w-full" onClick={() => setOpenScan(true)}>
              <span className="flex items-center gap-2">
                <LuTickets className="size-5" />
                Scan Tiket
              </span>
            </Button>
